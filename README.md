# 🌳 Livada Mea

**Ghid complet pentru livada de pomi fructiferi** — tăieri, anatomie, calendar și tehnici corecte.

🔗 **[Deschide aplicația →](https://tavib47.github.io/Livada/)**

---

## Ce conține

### 📋 [Tabel tăieri livadă](https://tavib47.github.io/Livada/tabel_taieri_livada.html)
Toți pomii din livadă, cu perioadele exacte de tăiere (primăvară și vară), recolta și observații esențiale. Filtrare pe specii.

### ✂️ [Ghid tăieri pe specii](https://tavib47.github.io/Livada/ghid_taieri_pomi.html)
Instrucțiuni detaliate pas cu pas pentru fiecare specie: formare, producție, reîntinerire și avertizări specifice.

### 🌿 [Anatomia pomului](https://tavib47.github.io/Livada/ghid_ramuri_pomi.html)
Ghid vizual complet: tipuri de ramuri, cum recunoști mugurii, tehnica tăieturii, pomi neglijați și cele mai frecvente greșeli.

---

## Specii acoperite

Măr · Păr · Cireș · Vișin · Cais · Piersic · Nectarin · Prun · Gutui · Kaki · Rodiu

**22 de soiuri** documentate cu perioade de tăiere, recoltă și observații specifice.

---

## Caracteristici

- 🌗 **Dark mode** — se adaptează automat la preferința sistemului
- 📱 **Responsive** — funcționează pe telefon, tabletă și desktop
- ⚡ **Zero dependențe** — HTML/CSS/JS pur, fără framework-uri sau build tools
- 🇷🇴 **Conținut în limba română**
