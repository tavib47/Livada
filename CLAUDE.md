...
- **informatii.html** — "Informații" page with two sections: (1) Interactive yearly calendar showing monthly tasks (pruning, harvesting, treatments, care) and (2) Filterable table of pruning schedules for 22 fruit tree varieties (10 species). JS handles section tabs, month selector, and species filter buttons.
...
- **Two design systems**: `index.html` and `ghid_ramuri_pomi.html` share a warm/earthy palette (`--bark`, `--leaf`, `--gold`, `--cream` vars). `informatii.html` and `ghid_taieri_pomi.html` share a neutral palette (`--bg`, `--text`, `--border` vars).
